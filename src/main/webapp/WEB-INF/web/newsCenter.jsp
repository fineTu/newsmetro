				...
				success : function(data) {
					...
					for(var j=0;j<data.length;j++){
						...
						for(t in targetList){
							...
							var ul = '<div id="data_'+targetList[t].id+'" ><ul class="fl" id="list_'+targetList[t].id+'"><li><img class="mt50" src="img/loading.gif" /></li></ul></div>';
		...
		function getTarget(isRss,targetId){
			if(isRss==0){
				$.ajax({
					...
					success : function(data) {
//						$("#block_"+targetId).html(data.html);
						/*$("#name_"+index).html("");
						 $("#name_"+index).append("<a class='a_link_blue' href='"+data.link+"' >" + targetList[index-1].name + "</a>");*/
						$("#a_update_"+targetId).remove();
						$("#description_"+targetId).append("<span>" + data.meta.description + "</span>");
						console.log(data.meta.description)
						$("#md5_"+targetId).attr("value",data.meta.md5);
						$("#data_"+targetId).html(data.html);
...
//						}
        ...
        function checkTargetUpdate(target_id){
            ...
            $.ajax({
                ...
                success : function(data) {
					if(data==null||data.md5==null|| $.trim(data.md5).length==0){
						return;
					}
